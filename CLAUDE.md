...
- `apps/web` — каркас SPA: i18n, сессия (access-токен в памяти, refresh в httpOnly cookie), вход с 2FA включая первичную настройку с QR, layout трёх ролей.

Тестов: 212 (172 pytest + 40 vitest), `make test` и `make lint` покрывают оба стека.

**Следующее по разделу 15** — пп. 10–14: экраны родителя (главная, калькулятор, продукты/рецепты, меню), `/logs` API и дневники, кабинет врача, админка, отчёты. `apps/bot` и `apps/worker` — только каркас, без логики: они относятся к этапам 3–4, не наполнять их «заодно» (правило 10 ниже).
