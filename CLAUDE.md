...
Тестов: 1017 pytest (`apps/api` 683, `.claude/hooks` 113, `packages/keto_engine` 67, `apps/bot` 82, `packages/core` 46, `apps/worker` 26) и 448 vitest (`apps/web` 343, `packages/ui` 70, `apps/miniapp` 30, `packages/api-client` 5). Единственный пропуск — рендер PDF без системных pango и cairo.
...
По боту пройден **продуктовый аудит глазами родителя** ([AUDIT_BOT.md](docs/AUDIT_BOT.md)):
