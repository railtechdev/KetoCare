...
- `packages/ui` — токены (раздел 8.2, включая тёмную тему для Mini App), `RatioBadge`, `MacroBar`, `WarningBanner`, `DiaryEntryCard`. `DataTable` и `TrendChart` — вместе с экранами, которые их используют.
...
- Экран **калькулятора** (раздел 8.3): три режима, поиск продуктов с автодополнением, результат через `RatioBadge`/`MacroBar`, неразрешимость показывается причиной, сохранение своего блюда.

Тестов: 290 (244 pytest + 46 vitest), `make test` и `make lint` покрывают оба стека.

**Следующее по разделу 15** — остаток п. 10 (продукты/рецепты, меню) и п. 11 (`/logs` API и дневники). Главная родителя ждёт `/logs`: по разделу 8.3 она показывает итоги дня, последние кетоны и вес. `apps/bot` и `apps/worker` — только каркас, без логики: они относятся к этапам 3–4, не наполнять их «заодно» (правило 10 ниже).
