...
Паттерны интерфейса `apps/web` / `apps/miniapp` — шаблон страницы, формы,
состояния (empty/loading/error/success), таблицы, мобильность, доступность —
зафиксированы в **`docs/UI_GUIDE.md`** (22 правила + чек-лист ревью экрана).
Любой новый или правленый экран проверяется по этому чек-листу; отступление от
канона обсуждается в PR, а не делается молча. Расположение кнопок, пометка
полей, поведение при отправке формы и т.п. берутся из канона — не спрашивать и
не изобретать.
