...
Два дефекта, из-за которых он не открывался, видны были только из настоящего
Telegram: страница не подключала `telegram-web-app.js` (без него нет
`window.Telegram.WebApp`, и приложение, открытое кнопкой в чате, считало себя
открытым вне Telegram), а строка запуска бралась у клиента только при
исключении SDK — молчаливую пустоту он не обрабатывал.
