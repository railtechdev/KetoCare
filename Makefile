...
miniapp: check-env ## Запустить Mini App (Vite); /api проксируется на API_PROXY_TARGET
	...
	@# нужен адрес снаружи (туннель): кнопка в боте иначе ведёт на localhost
	@# телефона. Самой кнопки пока нет — ей нужен выкат Mini App.
...
lint: openapi ## Линтеры и проверка типов (сначала генерирует api-client)
	...
	@# `infra` пока нет: там 37 ошибок, это отдельная работа.
