...
# `BACKUP_AGE_RECIPIENT` (публичный ключ age, приезжает секретом репозитория
# через deploy.yml); приватный ключ на сервере не хранится — иначе шифрование
...
if [ -z "$AGE_RECIPIENT" ] && [ -z "$ALLOW_PLAINTEXT" ]; then
  ...
  Публичный (age1…) — в СЕКРЕТ РЕПОЗИТОРИЯ BACKUP_AGE_RECIPIENT, а не в файл
  окружения на сервере: он пересобирается при каждом выкате и правку руками
  затирает — бэкап прекратился бы молча со следующим деплоем.
