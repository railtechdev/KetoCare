...
# Зашифрованный дамп расшифровывается во временный файл (`BACKUP_AGE_IDENTITY` —
# путь к приватному ключу). Ключ на сервере не хранится, поэтому проверка
# восстановления зашифрованной копии делается там, где он есть, — и это
# правильно: она заодно проверяет, что ключ ещё цел и подходит.
