...
# зависит, что попадёт в образ. Всё, что требует секретности, ставит свой umask
# в подоболочке — на один шаг, а не на весь скрипт.
umask 022
...
echo "→ окружение"
# umask в ПОДОБОЛОЧКЕ, и это не стилистика. Заданный на весь скрипт, он доживал
# до `git checkout`, и рабочее дерево ложилось правами 600. Файлы попадают в
# образ как есть, а процесс внутри контейнера идёт под непривилегированным
# пользователем — и просто не может их прочитать. Ровно так упал первый
# автодеплой:
#   PermissionError: [Errno 13] Permission denied: '/app/.../core/config.py'
#
# Сам umask нужен ДО создания файла, а не chmod после: между созданием и chmod
# файл с секретами был бы читаем всем.
( umask 077; cat > "$REPO/.env.next" )
...
# Имя ветки резолвится через origin, а не через локальную копию.
#
# Локальные ветки на сервере не двигаются: рабочее дерево живёт в отсоединённом
# HEAD, и `main` здесь застыл на том коммите, каким был при клонировании.
# `git checkout main` брал именно его — то есть выкат «ветки main» уезжал на
# код полугодовой давности вместе со СТАРЫМ `infra/scripts/deploy.sh`, который
# падал на `git pull` в отсоединённом HEAD. Автодеплой этого не замечал: он
# передаёт SHA прогона, а не имя ветки, и на грабли наступает только ручной
# запуск с `ref: main` — то есть человек, которому и так что-то чинить.
TARGET="$REF"
...
# Обновляем исполняемую копию точки входа СРАЗУ после checkout, а не в конце по
# успеху. Разница выяснилась на практике: ошибка в самом скрипте мешает деплою
# дойти до конца — а значит, мешает приехать собственному исправлению, и чинить
# приходится руками на сервере. Код здесь уже из коммита, прошедшего CI, так что
# ждать успеха выката незачем.
#
...
if [ -f "$REPO/infra/scripts/remote-deploy.sh" ] \
   && ! cmp -s "$REPO/infra/scripts/remote-deploy.sh" "$SELF"; then
    cp "$REPO/infra/scripts/remote-deploy.sh" "$SELF.next" \
        && chmod 0755 "$SELF.next" \
        && mv "$SELF.next" "$SELF" \
        && echo "точка входа обновлена — вступит в силу со следующего выката"
fi
