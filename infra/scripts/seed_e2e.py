...
def _require_credentials_on_allowed_host() -> None:
    ...
    if len(secret) % 8 not in _BASE32_BLOCK_TAILS:
        raise SystemExit(
            ...
            "восемь знаков кодируют пять байт, и остаток бывает только "
            + ", ".join(str(tail) for tail in sorted(_BASE32_BLOCK_TAILS))
            + ".\nСервер отвергнет такой секрет, а прогон — нет: вход упадёт\n"
