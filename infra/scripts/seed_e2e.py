...
#: другом хосте). Именно переменной, а не догадкой по строке подключения:
#: человек подтверждает, что понимает, какую учётку заводит.
_ALLOW_ANY_HOST = "E2E_SEED_ALLOW"


def _refuse_production(database_url: str) -> None:
    if os.environ.get(_ALLOW_ANY_HOST) == "1":
        return
    ...
    raise SystemExit(
        f"Адрес базы не похож на локальный (хост «{host or 'не указан'}»).\n"
        ...
        f"сквозной тест пишет назначение. Укажите локальную базу или задайте\n"
        f"{_ALLOW_ANY_HOST}=1, если это действительно ваша база."
    )
