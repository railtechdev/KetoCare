...
#: Разрешённые адреса: только своя машина и сервис-контейнер прогона.
#:
...
#: проверку проходил. Цена ошибки выросла: сид задаёт врачу не пустой второй
#: фактор, а ИЗВЕСТНЫЙ секрет из репозитория — на чужой базе это означает, что
#: второго фактора там больше нет. Защита должна расти вместе с риском, поэтому
#: неизвестный адрес теперь отвергается по умолчанию.
#: Только петля. Имён сервисов compose здесь НЕТ намеренно: боевой
...
#: порт на хост, поэтому разработчику имена сервисов не нужны.
_LOCAL_HOSTS = frozenset({"localhost", "127.0.0.1", "::1"})

#: Прежние признаки боевой строки — по ВСЕЙ строке подключения. Белый список по
#: хосту их не заменяет: туннель (`ssh -L 5432:postgres:5432`) делает чужую базу
#: локальной на вид, и тогда единственная зацепка — имя базы или пользователя.
_PRODUCTION_HINTS = ("railtech", "prod", "ketocare.uz")

#: Явное разрешение для случая, которого список не покрывает (своя база на
...
def _refuse(reason: str) -> NoReturn:
    raise SystemExit(
        ...
        "сквозной тест пишет назначение (отменить нельзя, prescriptions\n"
        "append-only). Укажите локальную базу или передайте разрешение\n"
        f"команде: {_ALLOW_HOST}=<хост> make seed-e2e.\n"
        "Именно команде, а не в `.env`: сид читает окружение процесса, и\n"
        "постоянная строка в файле отключала бы защиту навсегда."
    )
...
def _refuse_production(database_url: str) -> None:
    ...
    # чем соединяются.
    #
    # Путь в этом параметре — unix-сокет, он локален по определению, и такая
    # строка вообще не имеет хоста (без явного признания она упиралась бы в
    # отказ «хост не указан»). Но значение бывает СПИСКОМ через запятую — это
    # отказоустойчивость asyncpg: `host=/var/run,evil.com` разворачивается в
    # `['/var/run', 'evil.com']`, и не найдя сокета, драйвер идёт на второй
    # адрес. Поэтому путём считается только список, где КАЖДЫЙ элемент — путь.
    #
    ...
    # опечатка. Нормализуются обе стороны, иначе запрет обходится точкой, а
    # разрешение не срабатывает из-за пробела.
    host = (url.host or "").strip().rstrip(".").lower()
    if host in _LOCAL_HOSTS:
        return

    allowed = os.environ.get(_ALLOW_HOST, "").strip().rstrip(".").lower()
