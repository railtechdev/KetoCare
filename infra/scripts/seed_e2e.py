...
#: Алфавит base32 (RFC 4648). Знаки вне него — `0`, `1`, `8`, `9`, дефис,
#: знак равенства — ломают подсчёт кода: `apps/e2e/src/totp.ts` на них
#: отказывает разбирать секрет, и вход упал бы «Неверный код подтверждения»
#: без объяснимой причины. РЕГИСТР при этом не важен: `pyotp` декодирует с
#: `casefold=True`, а прогон сам приводит секрет к верхнему — отвергать
#: строчный значило бы отказывать рабочему значению.
...
def _require_credentials_on_allowed_host() -> None:
    ...
    if set(secret.upper()) - _TOTP_ALPHABET:
        raise SystemExit(
            ...
            "важен). Такой секрет не усилит второй фактор, а сломает подсчёт\n"
            "кода: вход упадёт «Неверный код подтверждения» без объяснимой\n"
            "причины."
        )
