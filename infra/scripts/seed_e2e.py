...
#:
#: Два исключения, и оба проверены исполнением, а не рассуждением. РЕГИСТР не
#: важен: `pyotp` декодирует с `casefold=True`, а прогон сам приводит секрет к
#: верхнему — отвергать строчный значило бы отказывать рабочему значению.
#: ПАДДИНГ (`=`) обе стороны тоже принимают: `totp.ts` срезает хвостовые `=`,
#: `pyotp` доклеивает их сам. Он отвергается по другой причине — знак
#: равенства считается в длину, и `"A" * 25 + "="` прошло бы как двадцать
#: шесть символов, неся 125 бит вместо 128.
...
def _require_credentials_on_allowed_host() -> None:
    ...
    if set(secret.upper()) - _TOTP_ALPHABET:
        raise SystemExit(
            ...
            "причины. Знак равенства принимают оба, но он считается в длину и\n"
            "потому запрещён здесь: с ним секрет короче, чем выглядит."
        )
