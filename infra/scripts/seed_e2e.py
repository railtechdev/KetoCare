...
#: рабочему значению.
#:
#: ЗНАК РАВЕНСТВА запрещён потому, что стороны на нём РАСХОДЯТСЯ: `totp.ts`
#: срезает хвостовые `=` и считает код, а `pyotp` на той же строке падает
#: (`Incorrect padding`) — он доклеивает паддинг сам и чужой ему мешает. Секрет,
#: с которым прогон входит, а сервер не пускает, хуже отсутствующего.
