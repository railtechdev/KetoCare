...
def _refuse_production(database_url: str) -> None:
    ...
    # Параметр `host` доезжает до драйвера и решает, куда идёт соединение. Путь
    # в нём — unix-сокет, а он локален по определению (подменить им адрес
    # нельзя); сетевое имя — подмена, и её отвергаем. Сокет при этом делает
    # подключение локальным целиком: у такой строки хоста нет вовсе, и без
    # явного разрешения она упиралась бы в отказ «хост не указан».
    socket_path = False
