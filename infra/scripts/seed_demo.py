...
_DANGER = (
    ...
    "репозитория и СБРАСЫВАЕТ второй фактор учёткам с теми же адресами — на\n"
    "чужой базе это открытая админка и снятая защита у живых людей."
)
...
def _refuse_production(database_url: str) -> None:
    ...
    Своё здесь только описание опасности и имя переменной. Имя службы боевого
    compose (`postgres`) разрешением открывается — в отличие от сида прогонов:
    демо-данные на стенд ставят по документу (docs/DEPLOY.md, «Демо-данные и
    фокус-группа»), и глухой запрет сломал бы законный путь, а не чужой.
    """
