...
| Термин | Значение |
|---|---|
| Кетосоотношение (ratio, R) | Отношение массы жиров к сумме масс белков и **чистых** углеводов: `F = R × (P + C − Fiber)`. Клетчатка вычитается по каждому продукту и не уходит в минус (утверждено клиникой 09.09.2026, ADR-0030). Лимит углеводов из назначения при этом считается по общим |
...
| Учёт углеводов | **утверждено клиникой 09.09.2026, ADR-0030:** соотношение — по чистым (за вычетом клетчатки), лимит `carbs_max_g` — по общим; переключателя `net_carbs` нет. До ответа было: общие, с заложенным переключателем |
