...
**Данные фокус-группы — уже реальные клинические данные, а способа удалить их
по просьбе участника в продукте пока нет.** ТЗ §11 и `CLAUDE.md` называют
команду `python -m core.tools.erase_patient <id>`, но она не написана: модуля
`core.tools` не существует. Пока его нет, удаление выполняется вручную по
запросу разработчика — и это надо сказать участникам до начала, а не после.
Долг зафиксирован в [ADR-0013](adr/0013-attachments-implementation.md).
