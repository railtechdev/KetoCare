...
| `ATTACHMENT_QUOTA_MB` | сколько всего вложений на одного пациента; по умолчанию 100 МБ — около двадцати сканов. Считать от размера диска: уборщика удалённых файлов пока нет |
...
Том `erased` копируется по той же причине и той же строкой (подставьте
`ketocare_erased`): после удаления по требованию участника это единственное, что
остаётся от его данных, а лежит оно ровно на том диске, который и отказывает.

Плюс копия за пределы VPS (rclone/scp — куда угодно, но не на этот же диск):
дамп, архив вложений и архивы удалений. Том `reports` копировать не нужно —
PDF-отчёты пересобираются из БД.
...
- Место на диске: старые образы копятся при каждом деплое —
  ...
  час на адрес. **Уборщика удалённых файлов нет** — байты мягко удалённого
  вложения остаются на диске (долг из [ADR-0013](adr/0013-attachments-implementation.md)),
  поэтому занятое место растёт монотонно.
