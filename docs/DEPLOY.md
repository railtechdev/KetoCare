...
**Повторное применение её не вернёт.** Заполнение при накате идёт по признаку
«анкету ни разу не правили», а к моменту повторного наката правки уже есть у
всех — ревизия пройдёт и оставит колонку пустой. Следующая за ней,
`e877386f5ebe`, добирает анкеты по началу терапии и тоже ничего не восстановит:
у ребёнка, который уже на диете, доисходных анкет не остаётся по определению.

Перед откатом ниже `e877386f5ebe` снимите значения — тогда их можно будет
вернуть вручную:
...
на кону. Опасен следующий шаг вниз.
