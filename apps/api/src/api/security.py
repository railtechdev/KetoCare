...
def verify_totp(secret: str | None, code: str, *, user_id: uuid.UUID | None = None) -> bool:
    ...
    Ранний выход — тотальность по ТИПУ, а не по значению: `None` роняет `pyotp`
    («object of type NoneType has no len()»), а пустая строка и без него даёт
    `False` (измерено). Поэтому мутация `not secret` → `secret is None` тестов не
    роняет, и так и должно быть. Опасна пустая строка в другом месте — вход
    считает фактор включённым по «поле не NULL» (`User.totp_enrolled`), и она
    проходит как включённый; это проверяется там, где ветвление и живёт.
