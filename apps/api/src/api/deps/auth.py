...
async def _assert_binding_alive(
    ...
    *,
    ...
    Запрос выполняется для каналов, открытых самой привязкой (`bot` и
    `miniapp`); веб-сессия за него не платит — она живёт паролем. Он же и
    причина, по которой `link_id` попадает в токен: иначе привязку пришлось бы
    искать по родителю и пациенту, а их у семьи может быть несколько, и отзыв
    одного чата гасил бы все.
    """
...
async def get_totp_setup_user(request: Request, session: SessionDep) -> CurrentUser:
    ...
    # Сессии, открытые привязкой чата (бот, Mini App), сюда не пускаются. Второй
    # фактор — личная вещь человека: с таким токеном можно было бы перевыпустить
    # TOTP-секрет родителя и превратить временный доступ к чату в постоянный
    # вход в кабинет (ADR-0009).
    if channel_of(payload) != "web":
        raise ApiError(
            ErrorCode.FORBIDDEN, "Настройка второго фактора доступна только в веб-кабинете."
        )
