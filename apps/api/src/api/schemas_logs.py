...
class MealLogCreate(LogCreate):
    """`parsed` через эту схему не принимается по-прежнему — и не будет.

    Подтверждение разбора приходит идентификатором задачи (`ai_job_id`), а
    структуру сервер берёт из журнала `ai_jobs` сам. Принимать её телом запроса
    значило бы разрешить клиенту прислать под видом разбора любые граммы и любые
    продукты — то есть клинические данные, которых модель не выдавала, а человек
    не подтверждал (правило 6 CLAUDE.md, раздел 10.3 ТЗ)."""
