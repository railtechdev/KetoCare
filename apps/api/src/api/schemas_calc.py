...
class VerifyResponse(BaseModel):
    dish: DishOut
    #: Соответствует ли соотношение цели. `null` означает ДВА разных случая, и
    #: различает их `ratio_comparable` ниже: цели не переданы (сравнивать не с
    #: чем) либо соотношения у блюда нет вовсе (сравнивать нечего, ADR-0038).
    #: Клиенты различали их по наличию цели у себя — то есть по догадке.
    ratio_within_tolerance: bool | None = None
    kcal_within_tolerance: bool | None = None
    #: Было ли с чем и что сравнивать: цели переданы И соотношение у блюда есть.
    #: `False` при `ratio_within_tolerance = null` значит «вердикта нет по
    #: существу», а не «мы его не спрашивали».
    ratio_comparable: bool = False
