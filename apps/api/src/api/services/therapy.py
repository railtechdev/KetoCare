...
async def check_therapy_start_is_plausible(
    ...
    **Будущая дата разрешена** и проверкой не считается: врач назначает диету «с
    ...
    решение до его исполнения нельзя. Так же устроено и правило про исходную
    частоту (`therapy.earliest_evidence_of_therapy`): оно сравнивает дату с
    сегодняшним днём, а не запрещает её.
