...
#: 99,98 г и белки 0,11 г, в сумме 100,09. Ошибки ввода, ради которых проверка
#: стоит, — килоджоули вместо килокалорий, перепутанные колонки — дают
#: расхождение в разы, а не в десятые, и полграмма их пропускать не начнёт.
...
def macro_sum_message(macro_sum: float) -> str:
    ...
    tolerance = f"{MACRO_SUM_ROUNDING_G:g}".replace(".", ",")
    return (
        f"Сумма жиров, белков и углеводов ({macro_sum:g} г) превышает 100 г на 100 г "
        f"продукта больше, чем допускает округление источника ({tolerance} г)."
    )
