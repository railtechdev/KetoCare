...
**Свободного текста, который пишет семья, нет ни одного поля.** Ни описаний
приступов, ни жалоб, ни прошлых сводок: `pseudonymize` снимает запрещённые
ключи, но внутрь строкового значения не смотрит, а «вечером Аня жаловалась на
тошноту» — это имя ребёнка в промпте (ADR-0019).

Два строковых поля всё же уходят — название препарата и доза. Их пишет
специалист, и без них раздел «Приверженность» пересказывать нечего. Контакты из
них снимает `pseudonymize` в клиенте модели: она чистит строковые значения на
любой глубине, а не только отдельный аргумент с текстом человека.
"""
