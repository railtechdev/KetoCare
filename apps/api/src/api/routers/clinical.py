...
Чтение и запись здесь разведены по ролям, а не идут одной проверкой на ручку.
Клиника ответила 09.09.2026 (вопросы 7 и 31): «Диетолог может видеть диагноз,
календарь приступов, назначения врача по АЭП, но не вносить изменения», и
отдельно — что назначать лекарства он права не имеет. Одна проверка на обе
операции заставила бы выбрать между «диетолог не видит того, чем кормит» и
«диетолог правит диагноз»; ни то, ни другое клиника не просила.
"""
...
# Раздел 5.3 ТЗ помечает и медицинский профиль, и схему терапии, и заметки как
# врачебные. Медицинская команда ответила иначе — и только про чтение.
#
# `_DOCTOR_ONLY` — всё, что МЕНЯЕТ клинические данные, плюс врачебные заметки
# целиком: заметка это личное свидетельство врача о приёме, и в ответе клиники
# её нет ни в чтении, ни в записи.
_DOCTOR_ONLY = Depends(require_roles(UserRole.DOCTOR))

# `_CARE_ROLES_READ` — чтение анамнеза ведущим специалистом. Ответ клиники от
# 09.09.2026 (вопрос 7): «Диетолог может видеть диагноз… но не вносить
# изменения». Диагноз определяет, какую диету вообще собирают, и подбирать
# рацион вслепую — это и был прежний порядок.
