...
async def login(
    ...
    if needs_totp:
        ...
        # ему «Неверный код подтверждения.» не за что.
        #
        # Раньше эта ветка сливалась со следующей, и цена была не только в
        # словах: клиент узнавал о необходимости второго фактора, ЛОВЯ ошибку, —
        # то есть каждый нормальный вход врача писал в журнал аудита
        # `login_failed_totp` и съедал две попытки из пяти в `AUTH_RATE_LIMIT`.
...
async def totp_verify(
    ...
    пользователь сразу получает рабочую пару токенов — кроме случая, когда
    пароль выдал администратор: тогда вместо пары приходит токен задания пароля.
    ...
    if db_user.password_change_required:
        # Пароль выдал администратор, и он его знает. Сессии здесь не выдаётся:
        # иначе первичная настройка второго фактора оказывалась обходом признака
        # — вход при живом временном пароле, не сменив его (#232). Проверка на
        # /auth/login до этой ветки не доходит: настройка фактора стоит раньше
        # неё и завершает вход сама.
        #
        # Резервные коды показываются всё равно: другого момента нет, в базе
        # лежит только sha256.
        return TotpEnabledResponse(
            backup_codes=codes,
            password_reset_token=create_token(
                user_id=db_user.id, role=db_user.role, token_type="password_reset"
            ),
        )
