...
// Все разделы семьи из ролевой таблицы кабинета (`SECTIONS_BY_ROLE.parent`).
// Список ведётся руками — `apps/e2e` не зависит ни от одного рабочего пакета,
// и тянуть реестр через границу пакета ради одной константы не стали, —
// поэтому его полноту сторожит тест на стороне кабинета
// (`apps/web/src/routes/mobileSections.test.ts`): он читает ЭТОТ файл и падает,
// когда у родителя появился раздел, сюда не попавший. Так уже случилось:
// «Помощник», «Ребёнок» и «Профиль» не проверялись на телефоне ни разу, хотя
// мобильный веб — основной канал семьи (раздел 8.2 ТЗ).
const SECTIONS = [
  "home",
  "menu",
  "diary",
...
test("экраны семьи помещаются в 360 px", async ({ page }) => {
  ...
  for (const section of SECTIONS) {
    ...
    // `networkidle` держится на том, что ни один экран семьи не опрашивает
    // сервер по кругу. Сегодня это так: опрос помощника включается только при
    // существующей переписке, а прогон их не заводит. Появится сценарий,
    // отправляющий вопрос помощнику, — воркера в прогоне нет, ответ навсегда
    // останется в ожидании, опрос не прекратится, и упадёт ЭТА проверка, а не
    // та, что сломалась.
