...
test("врач заводит карту и выдаёт код, семья входит по нему", async ({
  ...
  // Имя ребёнка семья видит в разделе «Ребёнок», а не на главной: главная
  // говорит о сегодняшнем дне, а единственного ребёнка не называет — его имя
  // там было бы повторением того, что и так известно. Проверять надо там, где
  // имя есть, иначе сценарий проверяет вёрстку главной, а не доступ.
  await family.goto("/app/child");
