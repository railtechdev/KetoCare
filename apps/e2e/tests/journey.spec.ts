...
test("врач назначает, семья ведёт день, отчёт сходится", async ({
  ...
  // Карта пациента — уровень адреса, а не параметр списка (ADR-0026). Прежний
  // `?patient=&tab=` живёт только как совместимость для закладок врачей, и её
  // проверяет `apps/web/src/router.test.ts`; сквозной сценарий обязан ходить по
  // тому адресу, который врач видит в строке браузера сегодня. (Семье карта
  // пациента не открывается вовсе: `patientRoute.beforeLoad` уводит родителя
  // на `/app/home`.)
  await doctor.goto(`/app/patients/${patient}/prescription`);
  ...
  await parent
    // Приём назван номером, а не «завтраком»: их в дне столько, сколько
    // назначил врач (ADR-0029).
    .getByRole("button", { name: "Добавить блюдо в приём: Приём 1" })
    .click();
  ...
  await doctor.goto(`/app/patients/${patient}/reports`);
