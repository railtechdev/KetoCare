...
export function CalculatorScreen({ session }: { session: Session }) {
  ...
  // не доработает. И пока данные не обновились: после УСПЕШНОГО повтора
  // фоновый перезапрос того же ввода (возврат сети) иначе выдавал себя за
  // повтор — над числами расчёта снова вставал и зачитывался прежний отказ.
  // После неудачного повтора такой перезапрос по-прежнему засчитывается
  // повтором — проверка при этом действительно идёт заново.
  ...
  // Отказал именно повтор: запрос снова в покое, и ошибок стало больше, чем
  // было при нажатии. Сверка счётчика — страховка: без неё отказ до нажатия
  // засчитывался бы как ответ повтора в любом кадре, где повтор уже записан, а
  // запрос ещё не стартовал. В тестах такой кадр не воспроизводится — запись
  // повтора и старт запроса попадают в одну отрисовку, — поэтому тестом она
  // не закреплена.
  const retryFailed =
    retryIsThisRequest &&
    verify.isError &&
    verify.fetchStatus === "idle" &&
