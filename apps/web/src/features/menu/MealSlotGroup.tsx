            ...
            return (
              ...
              >
                ...
                {/* Что и сколько взвесить. Раскрывается по требованию: у
                    ...
                    день и посчитан (ADR-0016). Граммовка приходит с сервера
                    уже масштабированной на позицию: клиент не знает числа
                    порций рецепта, и домножение здесь на `portion_factor`
                    завышало её в `servings` раз (находка М1 аудита Mini App). */}
                ...
                {/* Блюдо правили после того, как день сохранён. Сам день от
                    ...
                    правят, когда в нём нашли ошибку, и решать семье. */}
                {item.changed_since_saved && (
                  <p className="m-0 w-full text-sm text-muted-foreground">
                    {t("item.changedSinceSaved")}
                  </p>
                )}
