...
/**
 * Справочник противоэпилептических препаратов целиком.
 *
 * Отдаётся одной страницей и фильтруется на клиенте: строк около тридцати, и
 * серверный поиск ради них был бы лишним запросом на каждую букву.
 *
 * **`limit: 100` — потолок, а не «все».** Справочник ведёт медкоманда через
 * админку. Дойдёт до сто первой строки — подсказка в схеме лечения начнёт
 * терять препараты без единого признака, потому что `matchDrugs` фильтрует уже
 * ПОЛУЧЕННУЮ страницу. Тогда поиск надо переносить на сервер, как у продуктов;
 * границу стережёт тест в `useIntake.test.ts`.
 */
