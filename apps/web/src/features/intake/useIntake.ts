 ...
 *
 * **`limit: 100` — потолок, а не «все».** Справочник растёт миграциями. Дойдёт
 * до сто первой строки — подсказка в схеме лечения начнёт терять препараты без
 * единого признака, потому что `matchDrugs` фильтрует уже ПОЛУЧЕННУЮ страницу.
 * Тогда поиск надо переносить на сервер, как у продуктов; границу стережёт тест
 * в `useIntake.test.ts`.
 */
