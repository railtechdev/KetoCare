...
export function DiaryPage({ patientId }: { patientId: string }) {
  ...
  return (
    // Дневники — страница, на которой работают: сетка приступов «день x часть
    // суток» и список записей. В колонке 72rem сетка на месяц не помещалась и
    // уезжала в горизонтальную прокрутку, при том что справа было пусто.
    <PageLayout
      title={t("title")}
      intro={t("intro")}
      width="wide"
  ...
  return (
    <div className="flex flex-col gap-screen">
      ...
      {/* Сетка «день × часть суток» из дневника KETO-STEP: представление над
          ...
          Пустой сетки не бывает: когда записей нет, о пустоте говорит одно
          пустое состояние ниже. Раньше их было два подряд — блок «Дневник
