...
// Позиция состава — это `ProductOption`, каким его собирает `useProducts` из
// ответа справочника: суффиксы `_100g` остаются на границе с сервером, а в
// составе поля зовутся `kcal`, `fat`, `protein`. Прежняя фикстура описывала
// форму с суффиксами и глушила несоответствие через `as unknown as` — то есть
// проверка шла по данным, которых в кабинете не бывает.
const ROWS: DishRow[] = [
  {
    product: {
      id: PRODUCT_ID,
    ...
    await waitFor(() => {
      expect(api.POST).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          // Тело сверяется целиком, а не частью: сервер пересчитывает состав
          // сам по `product_id`, и клиентские макронутриенты рядом с ним были
          // бы вторым источником клинических чисел. `objectContaining` лишние
          // поля пропускал бы молча.
          body: {
            title: "Завтрак 4:1",
