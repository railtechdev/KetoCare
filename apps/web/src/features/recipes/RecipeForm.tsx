    ...
    >
      ...
      >
        ...
        <fieldset className="m-0 border-0 p-0">
          ...
          >
            {/* Имя из подсказки не сохраняется в строку: до ответа справочника
                она скажет «загружаем название…», а при отказе связи — «название
                не загрузилось», хотя человек только что видел имя в списке. Это
                размен: снимок имени в значениях формы застывал навсегда, потому
                что `defaultValues` читаются один раз при монтировании. Живое
                состояние дороже мгновенной подписи. */}
            <ProductPicker
