...
export function RecipeList({
  ...
  if (recipes.length === 0) {
    return <p className="text-muted">{t("list.empty")}</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {recipes.map((recipe) => (
          <li key={recipe.id}>
            <button
              ...
              className="flex h-full w-full flex-col overflow-hidden rounded-kc border border-line bg-surface text-left shadow-kc-sm"
            >
      ...
      <p role="status" className="m-0 text-sm text-muted tabular-nums">
      ...
      {recipes.length < total && (
        <button
          type="button"
          onClick={onShowMore}
          className="min-h-touch w-full max-w-xs rounded-lg border border-line px-4 text-ink"
        >
          {t("list.showMore")}
        </button>
      )}
