...
/**
 * Роль, которой сервер открывает клинические ручки пациента НА ЗАПИСЬ:
 * медицинский профиль (`PUT`), схема препаратов, врачебные заметки целиком
 * (`clinical.py`).
 *
 * **Чтение анамнеза — не отсюда, а из `isCareRole`:** диетолог видит диагноз и
 * назначенные препараты, но не правит их (ответы клиники на вопросы 7 и 31,
 * [ADR-0031](../../../../docs/adr/0031-dietitian-reads-clinical-history.md)).
 * Развести эти два права одним предикатом нельзя: получилось бы либо «диетолог
 * собирает рацион вслепую», либо «диетолог правит диагноз».
 *
