...
export interface PatientFlags {
  ...
  /**
   * Доступ семьи ещё не активирован: у ребёнка нет ни одного родителя.
   *
   * Карту заводит специалист на приёме (ADR-0040), и до того, как семья
   * активирует код, записей от неё не будет ни одной — это устройство продукта,
   * а не тревога. Без этого признака «семья молчит N дней» загоралось бы у
   * каждого только что заведённого пациента, то есть красным был бы весь
   * список в тот день, когда врач завёл приём.
   */
  familyNotActivated: boolean;
  /**
...
export function computePatientFlags(
  ...
  // Отсутствие поля (старый ответ API в секунды между выкатами) читается как
  // «семья есть»: так вело себя приложение до кодов доступа, и ложная пометка
  // хуже отсутствующей.
  const familyNotActivated = overview.family_activated === false;
  ...
  return {
    ...
    // Молчание не считается и там, где семья ещё не вошла: записывать некому,
    // и «семья молчит» было бы упрёком человеку, которого в системе нет.
    // Пометку о неактивированном доступе врач видит вместо него.
    staleData:
      !noPrescription &&
      !familyNotActivated &&
...
export function attentionRank(flags: PatientFlags | null): number {
  ...
  return (
    ...
    // Неактивированный доступ — не тревога, но и не пустяк: пока семья не
    // вошла, дневников не будет, и врач должен увидеть это раньше, чем начнёт
    // ждать записей. Вес ниже молчания: там данные ждут и не приходят, здесь
    // ещё никто не обещал их вносить.
    (flags.familyNotActivated ? 1 : 0) +
