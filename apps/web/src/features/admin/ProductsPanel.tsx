...
interface Props {
  /**
   * Импорт CSV доступен только администратору: `POST /products/import` закрыт
   * `require_roles(ADMIN)`, тогда как править каталог вправе и диетолог. Он
   * видит тот же список и ту же форму, но без этой двери — иначе упирался бы
   * в 403. История правок сюда НЕ относится: она открыта администратору,
   * диетологу и врачу (`_HISTORY_ROLES`) и потому включена безусловно.
   */
  canImport?: boolean;
  /**
...
export function ProductsPanel({
  ...
  if (item !== undefined) {
    ...
    // отдаёт панель с `canImport={false}` диетологу), оставлял экран
