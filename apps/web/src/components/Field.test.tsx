...
describe("поля формы", () => {
  it("подпись не распирает узкую колонку", () => {
    // Сторож для свойства, которое видно только на настоящем узком экране:
    // подпись кита — flex-контейнер, и её текст не уже самого длинного слова.
    // На 360 px «Кетосоотношение» (134 px в колонке 131 px) выталкивало
    // страницу вбок, а ловил это только ночной сквозной прогон — раз в сутки.
    // Проверка на класс слабее замера и не заменяет его, но падает сразу,
    // если перенос уберут: `break-words` здесь не годится (он не уменьшает
    // минимальную ширину содержимого), нужен именно `anywhere`.
    render(<Field id="ratio" label="Кетосоотношение" />);

    expect(screen.getByText("Кетосоотношение")).toHaveClass("wrap-anywhere");
  });
