...
if (typeof window !== "undefined") {
  ...
  // Списки с поиском (`Command` кита, cmdk) подводят выделенный пункт к краю
  // видимой области. `Element.scrollIntoView` в jsdom не реализован, и падение
  // выглядело обманчиво: список с пунктами рушился целиком, а пустой и список с
  // ошибкой проходили — то есть тест «не нашёл поле поиска» вместо «компонент
  // упал». В браузере прокрутка работает; заглушка закрывает пробел среды.
