...
Свободный текст человека чистит `scrub_free_text`: там нельзя убрать всё (имя
ребёнка в фразе «Аня съела два яйца» неотличимо от слова), но контакты — можно,
и они убираются (ADR-0019). Применяется она и к отдельному аргументу с текстом
человека, и к каждому строковому значению нагрузки: строки в нагрузке пишет
человек тоже — название препарата, доза, симптом, — и запретить их целиком
нельзя, они нужны модели.
...
def _as_json_value(value: Any) -> Any:
    ...
    if isinstance(value, str):
        # Контакты вырезаются и здесь, а не только из отдельного аргумента с
        # текстом человека. Строковые значения в нагрузке пишет человек тоже:
        # название препарата, доза, симптом. Ключ такого поля в списке запретов
        # не стоит и стоять не должен — значение нужно модели, — а контактам в
        # нём места нет (ADR-0019).
