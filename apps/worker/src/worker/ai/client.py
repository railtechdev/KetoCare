...
#:
#: Сводка врачу добавлена сюда позже. Одна сборка — это smart-модель на квартале
#: дневника, а бюджет общий: врач, перебирающий периоды, выжигает его за утро, и
#: первым замолкает помощник семей. Идемпотентность заказа от этого не спасает —
#: она закрывает повтор того же периода, но не перебор соседних.
USER_LIMITED_KINDS: tuple[AiJobKind, ...] = (AiJobKind.ASSISTANT, AiJobKind.DOCTOR_SUMMARY)
