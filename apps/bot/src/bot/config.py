...
class BotSettings(BaseSettings):
    ...
    # Адрес Mini App. Пусто — кнопки «Приложение» в меню нет вовсе: кнопка,
    # которая никуда не ведёт, хуже отсутствующей, а Telegram принимает в
    # web_app только https.
    miniapp_url: str = ""

    @property
    def has_miniapp(self) -> bool:
        """Есть ли у семьи кнопка «Приложение».

        Проверка одна на всех: клавиатура решает, показывать ли кнопку, а
        тексты — обещать ли её («откройте приложение 📱» в отбойнике и строка
        про приложение в /help). Две копии проверки однажды разошлись бы, и
        текст обещал бы кнопку, которой в меню нет.
        """

        return self.miniapp_url.strip().startswith("https://")
