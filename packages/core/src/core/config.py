...
class Settings(BaseSettings):
    ...
    # Интерактивная документация API (`/api/v1/docs`, `/api/v1/openapi.json`).
    #
    # По умолчанию включена: без неё разработка вслепую. На стенде выключается
    # (`API_DOCS=false` в окружении контейнера) — 139 маршрутов, схемы тел и коды
    # ошибок, читаемые кем угодно, это бесплатная карта целей. Спецификацию для
    # генерации клиента снимает не сервер, а `scripts/export_openapi.py`, так что
    # выключение ничего не ломает в сборке.
    api_docs: bool = True
    ...
    # База знаний помощника (раздел 10.4 ТЗ). Каталог лежит вне пакета — в
    # docs/, — поэтому путь до него настройка, а не константа: иначе воркер
    # находил бы базу только при запуске из корня репозитория.
    knowledge_base_dir: str = "./docs/knowledge-base"
    ...
    # Через сколько дней после удаления вложения уборщик снимает его байты с
    # диска. Отсрочка, а не немедленное стирание: родитель удаляет документ из
    # списка одним нажатием, и ошибка здесь необратима — выписка из стационара
    # существует в одном экземпляре.
    #
    # Тридцать дней — это срок хранения дампов из docs/DEPLOY.md: пока файл
    # можно достать из вчерашнего бэкапа, удаление обратимо руками. Медицинского
    # или юридического норматива на срок хранения удалённого документа у нас нет
    # — вопрос задан медкоманде (OPEN_QUESTIONS, вопрос про сроки хранения).
    attachment_purge_days: int = 30
