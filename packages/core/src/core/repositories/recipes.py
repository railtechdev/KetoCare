...
def _ratio_column() -> ColumnElement[float]:
    ...
    **Соотношение здесь — снимок, и снимки бывают разных версий ядра.** С 1.0.0
    оно считается по чистым углеводам (ADR-0030), а рецепты, посчитанные раньше,
    несут число по прежнему правилу и не пересчитываются. Значит подборка
    «3,5–4,0» частично идёт по одному правилу, частично по другому, и рецепт с
    клетчаткой может в неё не попасть, хотя по сегодняшнему правилу попадал бы.
    Пересчитать рецепт можно ручкой `POST /recipes/{id}/recompute`; массового
    пересчёта не делалось — это решение о данных, а не о коде.
    """
