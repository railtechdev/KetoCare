...
class User(Base, UUIDPkMixin, CreatedAtMixin, UpdatedAtMixin):
    ...
    def has_totp(self) -> bool:
        """Второй фактор настроен И пригоден к проверке кода.
        ...
        У каждого вопроса о втором факторе ОДИН предикат, и все они здесь:
        включён (`totp_enrolled`), пригоден (этот), есть что сбрасывать
        (`totp_resettable`). Прежде на три вопроса приходилось три случайных
        выражения, разбросанных по роутеру и сервису, и на испорченном секрете
        карточка администратора говорила одно, а вход — другое (#219).
        """
