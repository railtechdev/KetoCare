...
class User(Base, UUIDPkMixin, CreatedAtMixin, UpdatedAtMixin):
    ...
    def has_totp(self) -> bool:
        ...
        Предикат ОДИН на всё приложение. Прежде их было три (`bool` на входе,
        `is not None` в настройке и здесь), и на испорченном секрете карточка
        администратора говорила «настроен», а вход — «не настроен»; по этой же
        карточке администратор решает, сбрасывать ли фактор (#219).
        """
