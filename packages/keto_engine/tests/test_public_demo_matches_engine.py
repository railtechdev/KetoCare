...
Саму формулу лендинга проверяет `packages/landing/src/lib/keto.test.ts`: этот
тест её не читает, тот не видит смены медицинского правила, и нужны оба.

**Да, тест ядра читает файл соседнего пакета** — и обязательный отдельный
CI-job `make test-engine` покраснеет от правки маркетингового ассета. Это
сделано намеренно: растяжка нужна ровно там, где стоит, а ядро — та сторона,
которая знает правило. Переносить её в лендинг нельзя: ядро на Python.
"""
