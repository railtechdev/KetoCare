 ...
 *
 * `T` обязан быть JSON-устойчивым: ключ считается от `JSON.stringify`, поэтому
 * `undefined` и `null` дают один ключ, а у объекта он зависит от порядка полей.
 * Скаляр или `null` — то, для чего хук и сделан.
 */
