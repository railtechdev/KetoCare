    ...
    >
      <CardHeader className={cn(compact && "px-block")}>
        {/* `min-w-0 break-words`: шапка карточки — сетка (`1fr auto` при
            ...
            сжимался, а распирал блок. `hyphens-auto`: на телефоне обычное
            слово рядом с кнопкой («Схема лекарственной терапии») теперь
            переносится, а без него рвалось посередине без дефиса. */}
        <CardTitle
          className={cn(
